...
func saveConfigEnv(path string, env map[string]string) error {
	...
	// Keys the setup wizard doesn't prompt for are still written back.
	// EMAIL_HASH_SALT is the case that matters: `chb members sync` mints one
	// on first run and saves it here, but a whitelist-only writer silently
	// dropped it — so every sync generated a *fresh* salt, every emailHash
	// changed, and no member could be matched from one month to the next.
	// Anything the file already holds is data someone meant to keep.
	var extra []string
	for key, val := range env {
		if known[key] || strings.TrimSpace(val) == "" {
			continue
		}
		extra = append(extra, fmt.Sprintf("%s=%s", key, val))
	}
	sort.Strings(extra)
